...
export default function PlanDisplay({ plan }: { plan: BusinessPlan }) {
  ...
  const r = plan.risks
  const a = plan.action_items
  const qa = plan.qa_review
  ...
  return (
    <div style={{ background: "hsl(240,25%,4%)", minHeight: "100vh" }}>
      ...
      <div className="max-w-3xl mx-auto px-6 pt-28 pb-20">
        ...
        {a && (
          <div className="rounded-2xl p-6 mb-6"
            style={{ background: "hsl(240,15%,8%)", border: "1px solid rgba(255,255,255,0.06)", borderLeft: "4px solid hsl(142,71%,45%)" }}>
            <p className="text-xs uppercase tracking-widest mb-4" style={{ color: "rgba(255,255,255,0.35)" }}>Action Plan</p>
            {a.investor_one_liner && (
              <p className="text-sm mb-5 pl-3" style={{ borderLeft: "2px solid hsl(258,85%,64%)", color: "rgba(255,255,255,0.8)", fontStyle: "italic" }}>
                “{a.investor_one_liner}”
              </p>
            )}
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-4">
              {([["First 30 days", a.next_30_days], ["By day 60", a.next_60_days], ["By day 90", a.next_90_days]] as [string, string[]][]).map(([label, items]) => (
                <div key={label} className="rounded-xl p-4" style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)" }}>
                  <p className="text-xs font-semibold mb-2 uppercase tracking-widest" style={{ color: "hsl(258,80%,72%)" }}>{label}</p>
                  <ul className="space-y-1.5">
                    {items?.map((it, i) => (
                      <li key={i} className="text-xs" style={{ color: "rgba(255,255,255,0.7)" }}>▸ {it}</li>
                    ))}
                  </ul>
                </div>
              ))}
            </div>
            {a.recommended_kpis && a.recommended_kpis.length > 0 && (
              <div className="flex flex-wrap gap-2">
                {a.recommended_kpis.map((k, i) => (
                  <span key={i} className="text-xs px-2 py-1 rounded-full"
                    style={{ background: "rgba(34,197,94,0.1)", color: "rgb(74,222,128)", border: "1px solid rgba(34,197,94,0.2)" }}>📊 {k}</span>
                ))}
              </div>
            )}
          </div>
        ...
        {qa && (
          <div className="rounded-2xl p-6 mb-6"
            style={{ background: "hsl(240,15%,8%)", border: "1px solid rgba(255,255,255,0.06)", borderLeft: "4px solid rgb(125,211,252)" }}>
            <div className="flex items-center justify-between mb-4">
              <p className="text-xs uppercase tracking-widest" style={{ color: "rgba(255,255,255,0.35)" }}>QA Critic Review</p>
              <div className="flex items-center gap-2">
                <span className="text-xl font-bold" style={{ color: qa.overall_score >= 7 ? "rgb(74,222,128)" : "rgb(250,204,21)" }}>{qa.overall_score}/10</span>
                <span className="text-xs px-2 py-0.5 rounded-full"
                  style={{ background: qa.investment_ready ? "rgba(34,197,94,0.15)" : "rgba(234,179,8,0.15)", color: qa.investment_ready ? "rgb(74,222,128)" : "rgb(250,204,21)" }}>
                  {qa.investment_ready ? "Investment-ready" : "Needs work"}
                </span>
              </div>
            </div>
            {qa.verdict && <p className="text-sm mb-4 italic" style={{ color: "rgba(255,255,255,0.75)" }}>“{qa.verdict}”</p>}
            <div className="grid grid-cols-2 gap-3">
              <div><p className="text-xs font-semibold mb-1 uppercase tracking-widest" style={{ color: "rgb(74,222,128)" }}>Strengths</p>
                <ul className="space-y-1">{qa.strengths?.map((s, i) => <li key={i} className="text-xs" style={{ color: "rgba(255,255,255,0.65)" }}>+ {s}</li>)}</ul></div>
              <div><p className="text-xs font-semibold mb-1 uppercase tracking-widest" style={{ color: "rgb(252,165,165)" }}>Weaknesses</p>
                <ul className="space-y-1">{qa.weaknesses?.map((w, i) => <li key={i} className="text-xs" style={{ color: "rgba(255,255,255,0.65)" }}>− {w}</li>)}</ul></div>
            </div>
          </div>
        )}
